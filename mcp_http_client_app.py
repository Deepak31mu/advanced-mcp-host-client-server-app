...
class MCPHTTPClientApp(MCPHTTPClient):
    ...
    def __init__(self, server_url: str, roots_dir: str):
        ...
        self.tools_cache = []
        self.prompts_cache = []
        logger.info(f"Initialized GUI Client App for server: {server_url}")
    ...
    async def gui_call_tool(self, tool_name, arguments_json):
        ...
        await self.connect()
        if not tool_name:
            logger.warning("Tool call attempted without tool selection")
...
def main():
    if len(sys.argv) < 3:
        ...
        print("  MCP_GUI_HOST - GUI host (default: 127.0.0.1)")
        print("  MCP_GUI_PORT - GUI port (default: 7861)")
        sys.exit(1)

    server_url = sys.argv[1]
    roots_dir = sys.argv[2]
    
    gui_host = os.getenv('MCP_GUI_HOST', '127.0.0.1')
    gui_port = int(os.getenv('MCP_GUI_PORT', 7861))

    logger.info(f"Starting GUI Client App on http://{gui_host}:{gui_port}")
    print(f"Starting GUI Client App on http://{gui_host}:{gui_port}")
    
    client = MCPHTTPClientApp(server_url, roots_dir)
    interface = client.create_interface()
    interface.queue().launch(server_name=gui_host, server_port=gui_port)
